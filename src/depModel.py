...
class DepModel:
    def __init__(self, vocab, net_properties, trained_model_path):
        '''
        ...
        self.network.load(trained_model_path)
    ...
    def score(self, str_features):
        ...
        result = self.network.build_graph(str_features)
        return result.value()

if __name__=='__main__':
    
    # loading vocab and net properties
    with open(sys.argv[3], 'r') as vptr:
        vocab, net_properties = pickle.load(vptr)
    
    trained_model_path = os.path.abspath(sys.argv[4])
    
    m = DepModel(vocab, net_properties, trained_model_path)
