...
class Network:
    def __init__(self, vocab, properties):
        ...
        # define the hidden layer 1 bias term and initialize it as constant 0.2.
        self.hidden_layer_bias_1 = self.model.add_parameters(properties.hidden_dim_1, init=dynet.ConstInitializer(0.2))

        # define the hidden layer 2 bias term and initialize it as constant 0.2.
        self.hidden_layer_bias_2 = self.model.add_parameters(properties.hidden_dim_2, init=dynet.ConstInitializer(0.2))

        # define the output weight.
        self.output_layer = self.model.add_parameters((vocab.num_actions(), properties.hidden_dim_2))
    ...
    def build_graph(self, features):
        ...
        tag_ids = [self.vocab.feat_tag2id(tag_feat) for tag_feat in features[20:40]]
        label_ids = [self.vocab.feat_label2id(label_feat) for label_feat in features[40:52]]
        ...
        label_embeds = [self.label_embedding[did] for did in label_ids]
        ...
        hidden_1_dropout = dynet.dropout(hidden_1, 0.25)

        #hidden_2 = self.transfer(self.hidden_layer_2.expr() * hidden_1 + self.hidden_layer_bias_2.expr())
        hidden_2 = self.transfer(self.hidden_layer_2.expr() * hidden_1_dropout + self.hidden_layer_bias_2.expr())
        hidden_2_dropout = dynet.dropout(hidden_2, 0.25)

        # calculating the output layer
        #output = self.output_layer.expr() * hidden_2 + self.output_bias.expr()
        output = self.output_layer.expr() * hidden_2_dropout + self.output_bias.expr()
